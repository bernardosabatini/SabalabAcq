function runTraceAnalyzer(phys)
    ...
    for lineCounter=1:size(state.analysis.setup)
		...
		if state.analysis.setup{lineCounter, 2}==state.phys.internal.pulseUsed0 | state.analysis.setup{lineCounter, 2}==0
			if state.analysis.setup{lineCounter, 1} < 10  & phys
				...
				if state.analysis.setup{lineCounter, 1} == 1 
					...
					% acquired
					for channelCounter=state.phys.internal.acquiredChannels
						traceNames{end+1}=physTraceName(channelCounter, state.files.lastAcquisition);
						pulsePrefixes{end+1}=['AD' num2str(channelCounter)];
					end
				elseif any((state.analysis.setup{lineCounter, 1}-2)==state.phys.internal.acquiredChannels)	
		...
		%							tracePeaks(peakCounter)=evalin('base', [name '.xscale(2)*sum(' name '.data(' num2str(bStart) ':' num2str(bEnd) ')-' num2str(bVal) ')']);
									dX=state.analysis.setup{lineCounter, 7}(peakCounter, 3)-state.analysis.setup{lineCounter, 7}(peakCounter, 2);
                                %    disp(['(' name '.data(' num2str(bEnd) ')-' name '.data(' num2str(bStart) '))/' num2str(dX)]);

                                    tracePeaks(peakCounter)=evalin('base', ['(' name '.data(' num2str(bEnd) ')-' name '.data(' num2str(bStart) '))/' num2str(dX)]);
							end
