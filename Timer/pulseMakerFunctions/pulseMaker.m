...
% Last Modified by GUIDE v2.5 08-Dec-2010 15:49:58
...
function varargout = changePulse_Callback(h, eventdata, handles, varargin)
	...
	if eval(['ischar(state.pulses.' tag ')'])
		eval(['state.pulses.' tag 'List{state.pulses.patternNumber} = state.pulses.' tag ';']);
	else
		eval(['state.pulses.' tag 'List(state.pulses.patternNumber) = state.pulses.' tag ';']);
	end
	makePulsePattern;
	
    if any(state.pulses.patternNumber == [...
	...
	try
		if state.blaster.active && ...
				any(state.blaster.allConfigs{state.blaster.currentConfig, 2}(:, 2) == state.pulses.patternNumber)
